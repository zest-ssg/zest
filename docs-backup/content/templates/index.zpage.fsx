...
// @permalink /templates/
// @tags   模板, fsharp, 参考
// @description .zpage.fsx 模板语言完整参考 — 元数据、HTML DSL、短代码、Collections API

# .zpage.fsx 模板语言

`.zpage.fsx` 是 Zest 的原生模板格式——**F# 脚本 + Markdown + HTML DSL** 三合一。
...
适合不需要 Markdown 或 F# 逻辑的静态页面，构建速度最快，不经 FSI 编译：
...
若 `_config.toml` 中设置了 `template_engine = "nunjucks"`，可在 `.zhtml` 中使用模板语法：
...
stylesheet "/assets/css/style.zcss"                    // 外部 .zcss
```
