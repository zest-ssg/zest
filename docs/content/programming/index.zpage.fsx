...
render [
    ...
    blockquote [
        p [ text "「Zest 的模版即代码——就像 11ty.js 中 JavaScript 模版在构建时执行，Zest 的 .zpage.fsx 文件是真实 F# 脚本，在构建时编译求值。」" ]
    ]

    h2 [ text "概述" ]
    p [ text "Zest 的 .zpage.fsx 文件不仅仅是模版——它们是真正的 F# 脚本。" ]
    ...
    h2 [ text "页面结构" ]
    p [ text "一个典型的 .zpage.fsx 文件由两部分组成：" ]
    h3 [ text "1. 元数据注释（// @ 前缀）" ]
    ...
    h2 [ text "完整示例" ]
    p [ text "下面是一个完整的 .zpage.fsx 页面文件：" ]
