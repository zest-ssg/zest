...
render [
    ...
    ol [
        ...
        li [ p [ text "创建 FsiEvaluationSession（FSharp.Compiler.Service 交互式会话）" ] ]
        li [ p [ text "执行 #r 指令加载 Zest.Engine.dll，执行 open 导入命名空间" ] ]
        li [ p [ text "定义 render 辅助函数和全局数据访问函数" ] ]
        li [ p [ text "调用 EvalExpression 执行脚本，获取返回的 HTML 字符串" ] ]
