...
module HtmlElements =
    ...
    let inputCheckbox n v checked= Element("input", ["type", "checkbox"; "name", n; "value", v; (if checked then "checked" else ""), "checked"], [])
    let inputRadio n v checked    = Element("input", ["type", "radio"; "name", n; "value", v; (if checked then "checked" else ""), "checked"], [])
